...
void print_error(short rc)
{
    switch(rc)
    {
        ...
        case STRING_OVERFLOW:
            printf("\nОШИБКА! Переполнение возможно допустимых введенных значений!\n");
            break;
        case EPS_ERR:
            printf("\nОШИБКА! В числе находятся несколько обозначений \"e\" либо только \"e\"!\n");
            break;
        case INCORRECT_NUM:
            printf("\nОШИБКА! Некорректно записано вещественное число!\n");
            break;
        case MANTISSA_SIGN_ERR:
            printf("\nОШИБКА! Записано несколько знаков перед мантиссой или в самой мантиссе (знак должен быть один)!\n");
            break;
        case MANTISSA_POINT_ERR:
            printf("\nОШИБКА! В вещественном числе находятся несколько точек (в мантиссе)!\n");
            break;
        case MANTISSA_OVERFLOW:
            printf("\nОШИБКА! В мантиссе больше 30 элементов!\n");
            break;
        case ORDER_SIGN_ERR:
            printf("\nОШИБКА! Записано несколько знаков перед порядком или в самом порядке (знак должен быть один)!\n");
            break;
        case ORDER_POINT_ERR:
            printf("\nОШИБКА! В порядке не должно быть точек!\n");
            break;
        case ORDER_OVERFLOW:
            printf("\nОШИБКА! В порядке больше 5 цифр!\n");
            break;
        case ORDER_INCORRECT:
            printf("\nОШИБКА! Неверно указан порядок!\n");
            break;
        default:
            break;
    }
